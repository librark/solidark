...
it('compileRobotDefinition compiles robot metadata and geometry descriptors', async () => {
  const [robotElement] = parseMarkup(`
    <sol-robot name="arm">
      ...
      <sol-robot-actuator name="elbow_motor" joint="base_to_elbow" kind="servo"></sol-robot-actuator>
  ...
  assert.deepEqual(robot.actuators, [{ name: 'elbow_motor', joint: 'base_to_elbow', kind: 'servo' }])
...
it('compileRobotDefinition reports deterministic diagnostics and default names', async () => {
  const [robotElement] = parseMarkup(`
    <sol-robot>
      ...
      <sol-robot-joint name="fixed_joint" axis="1"></sol-robot-joint>
      <sol-robot-actuator joint="missing_joint"></sol-robot-actuator>
