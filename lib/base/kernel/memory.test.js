...
it('creates in-memory descriptor shapes', () => {
  ...
  const sketch = kernel.sketch({}, [
    ...
    kernel.arc({ through: [1, 1], point: [0, 1] }),
    kernel.close()
  ])
  const transform = kernel.translate({ by: [1, 0, 0] }, [primitive])
  const styling = kernel.color({ color: '#336699' }, [transform])
  const operation = kernel.union({}, [styling])
  ...
  assert.equal(styling.children[0], transform)
  assert.equal(operation.children[0], styling)
