...
it('ThreeCadRenderer applies initial CAD viewer options', () => {
  const target = createCadTarget()
  const renderer = new ThreeCadRenderer(target, [sampleMesh()], {
    axesVisible: false,
    cameraMode: 'perspective',
    edgesVisible: true,
    fitMargin: 3,
    gridVisible: false,
    three: createFakeThreeForCad(),
    view: 'right',
    xray: true
  })

  assert.equal(renderer.axes.visible, false)
  assert.equal(renderer.camera, renderer.perspectiveCamera)
  assert.equal(renderer.grid.visible, false)
  assert.equal(renderer.meshEntries[0].edges.visible, true)
  assert.equal(renderer.meshEntries[0].surface.material.options.transparent, true)
  assert.equal(renderer.meshEntries[0].surface.material.options.opacity, 0.38)
  assert.equal(renderer.state.theta, Math.PI / 2)
  assert.equal(renderer.state.phi, Math.PI / 2)
  assert.equal(renderer.state.radius, renderer.bounds.size * 3)
  assert.equal(renderer.view('unknown'), renderer)
  assert.equal(renderer.state.theta, -Math.PI / 4)
  assert.equal(renderer.state.phi, Math.PI / 3)
  assert.equal(renderer.fit(), renderer)
  assert.equal(renderer.state.radius, renderer.bounds.size * 3)
  renderer.dispose()
})

it('ThreeCadRenderer falls back from invalid initial CAD viewer options', () => {
  const renderer = new ThreeCadRenderer(createCadTarget(), [sampleMesh()], {
    cameraMode: 'bad',
    fitMargin: -1,
    three: createFakeThreeForCad(),
    view: 'bad'
  ...
  assert.equal(renderer.camera, renderer.orthographicCamera)
  assert.equal(renderer.state.radius, renderer.bounds.size * 2.2)
  assert.equal(renderer.state.theta, -Math.PI / 4)
  assert.equal(renderer.state.phi, Math.PI / 3)
  assert.equal(renderer.meshEntries[0].surface.material.options.transparent, false)
  renderer.dispose()
})
