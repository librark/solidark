...
import { canUseThreeTarget, createThreeCadScene } from './cad.js'
...
export class Viewer {
  ...
  render (result) {
    ...
    if (this.target) {
      ...
      let skipModelViewer = false

      if (meshes.length > 0 && canUseThreeTarget(this.target, this.options)) {
        try {
          this.renderer = createThreeCadScene(this.target, meshes, this.options)
          return this
        } catch {
          skipModelViewer = true
          this.renderer = null
        }
      }

      if (meshes.length > 0 && !skipModelViewer && canUseModelViewerTarget(this.target)) {
 ...
 * @returns {Viewer}
 */
export function createViewer (target = null, options = {}) {
