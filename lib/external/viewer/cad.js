...
const CAD_VIEWER_STYLE_ID = 'solidark-cad-viewer-styles'
const CAD_VIEWER_STYLES = `
.solidark-cad-viewer,
.solidark-cad-viewer *,
.solidark-cad-viewer *::before,
.solidark-cad-viewer *::after {
...
.solidark-cad-toolbar .solidark-cad-button {
  appearance: none;
  align-items: center;
  background: #ffffff;
...
.solidark-cad-toolbar .solidark-cad-button:hover {
  background: #e0f2fe;
  border-color: #38bdf8;
}

.solidark-cad-toolbar .solidark-cad-button:focus-visible {
...
.solidark-cad-canvas:active {
  cursor: grabbing;
}

.solidark-cad-axis-legend {
  align-items: center;
...
.solidark-cad-axis-legend.is-disabled {
  opacity: 0.58;
}

.solidark-cad-axis-label {
  align-items: center;
  color: var(--solidark-axis-color);
  cursor: pointer;
  display: inline-flex;
  font-family: "SFMono-Regular", Consolas, "Liberation Mono", monospace;
  font-size: 12px;
...
.solidark-cad-axis-label:focus-visible {
  outline: 2px solid #38bdf8;
  outline-offset: 3px;
}

.solidark-cad-axis-label::before {
  background: var(--solidark-axis-color);
  border-radius: 999px;
  content: "";
...
export function canUseThreeTarget (target, options = {}) {
  ...
  const three = resolveThree(options)
  const hasWebGl = options.three || (view?.WebGLRenderingContext && canCreateWebGlContext(document))
...
export class ThreeCadRenderer {
  constructor (target, meshes = [], options = {}) {
    this.target = target
    this.meshes = meshes
    this.three = resolveThree(options)
    if (!this.three) {
      throw new TypeError('Three.js is required to create the Solidark CAD viewer')
    }
    this.document = target.ownerDocument || globalThis.document
    this.window = this.document?.defaultView || globalThis
    ensureCadViewerStyles(this.document)
    this.bounds = meshBounds(meshes)
    this.state = {
      mode: cameraModeFromOptions(options),
      target: this.bounds.center,
      radius: this.bounds.size * fitMarginFromOptions(options),
      theta: viewAnglesFromOptions(options)[0],
      phi: viewAnglesFromOptions(options)[1]
    }
    this.drag = null
    this.axesVisible = options.axesVisible !== false
    this.edgesVisible = options.edgesVisible === true
    this.fitMargin = fitMarginFromOptions(options)
    this.gridSize = viewerGridSize(options.gridSize, this.bounds)
    this.gridVisible = options.gridVisible !== false
    this.xray = options.xray === true
  ...
  view (name) {
    const [theta, phi] = viewAngles(name)
  ...
  fit () {
    ...
    this.state.radius = this.bounds.size * this.fitMargin
    this.render()
    return this
  }
...
function resolveThree (options = {}) {
  return options.three || globalThis.SolidarkThree || globalThis.THREE
}

function ensureCadViewerStyles (document) {
  /* node:coverage ignore if */
  if (!document?.createElement) {
    return null
  }

  const existing = document.getElementById?.(CAD_VIEWER_STYLE_ID)
  ...
  style.id = CAD_VIEWER_STYLE_ID
  style.textContent = CAD_VIEWER_STYLES
...
function cameraModeFromOptions (options) {
  return options.cameraMode === 'perspective' ? 'perspective' : 'orthographic'
}

function fitMarginFromOptions (options) {
  const margin = Number(options.fitMargin ?? 2.2)

  return Number.isFinite(margin) && margin > 0 ? margin : 2.2
}

function viewAnglesFromOptions (options) {
  return viewAngles(options.view || 'isometric')
}

function viewAngles (name) {
  const views = {
    front: [0, Math.PI / 2],
  ...
  return views[name] || views.isometric
}
