...
export class ViewerComponent extends Component {
  ...
  static observedAttributes = ['axes', 'camera', 'edges', 'fit-margin', 'for', 'grid', 'grid-size', 'view', 'xray']
  ...
  get viewerOptions () {
    const { axes, camera, edges, fitMargin, grid, gridSize, view, xray } = this.properties

    return {
      axesVisible: viewerBoolean(axes, true),
      cameraMode: viewerCameraMode(camera),
      edgesVisible: viewerBoolean(edges, false),
      fitMargin: viewerFitMargin(fitMargin),
      gridSize: viewerGridSize(gridSize),
      gridVisible: viewerBoolean(grid, true),
      view: viewerView(view),
      xray: viewerBoolean(xray, false)
    }
  }
}
...
function viewerBoolean (value, defaultValue) {
  if (value === undefined) {
    return defaultValue
  }

  return ![false, 0, 'false', 'off', 'no', '0'].includes(value)
}

function viewerCameraMode (value) {
  return value === 'perspective' ? 'perspective' : 'orthographic'
}

function viewerFitMargin (value) {
  const margin = Number(value ?? 2.2)

  return Number.isFinite(margin) && margin > 0 ? margin : 2.2
}

function viewerView (value) {
  return ['front', 'isometric', 'right', 'top'].includes(value) ? value : 'isometric'
}
