...
it('detects whether a target can use the Three.js CAD renderer', () => {
  const previousThree = globalThis.SolidarkThree
  const document = createFakeDocument()
  const target = createTarget({ document })

  try {
    globalThis.SolidarkThree = createFakeThree()
    ...
    globalThis.SolidarkThree = previousThree
  }
})

it('requires an injected or global Three.js module to create the CAD renderer', () => {
  assert.throws(
    () => createThreeCadScene(createTarget({ document: createFakeDocument() }), [sampleMesh()]),
    /Three\.js is required/
  )
})

it('fails normally when a target document cannot create viewer elements', () => {
  const document = {
    defaultView: createFakeWindow()
  }

  assert.throws(
    () => createThreeCadScene(createTarget({ document }), [sampleMesh()], { three: createFakeThree() }),
    /createElement/
  )
})
...
it('renders meshes into a Three.js CAD viewport with controls', () => {
  ...
  assert.equal(document.head.children[0].id, 'solidark-cad-viewer-styles')
  assert.match(document.head.children[0].textContent, /\.solidark-cad-toolbar \{/)
  assert.match(document.head.children[0].textContent, /flex-wrap: wrap;/)
  assert.match(document.head.children[0].textContent, /\.solidark-cad-toolbar \.solidark-cad-button \{/)
  assert.match(document.head.children[0].textContent, /width: auto;/)
  assert.match(document.head.children[0].textContent, /flex: 0 0 auto;/)
  ...
  const second = createThreeCadScene(createTarget({ document }), [sampleMesh()], { three })
...
it('uses global document and the default Three.js module when not injected', () => {
  ...
  try {
    ...
    assert.equal(scene.three.Scene.name, 'FakeGroup')
    assert.equal(target.children[0].className, 'solidark-cad-viewer')
    scene.dispose()
  } finally {
