...
function makeSketchWire (openCascade, children) {
  return makeWireFromSegments(openCascade, sketchSegments(children))
}
...
function makeExtrudedShape (openCascade, properties, children) {
  const profile = makeProfileShape(openCascade, properties, children)
...
function makeSweepShape (openCascade, properties, children) {
  ...
  if (values.length < 2) {
    return { value: values[0] || null, handles: [] }
  }
...
function makeProfileShape (openCascade, properties, children) {
  const value = firstShapeValue(children)
