...
import { parseTopologyNames, resolveTopologySelector } from '../topology-selector.js'
...
export class OpencascadeKernel extends MemoryKernel {
  ...
  cylinder (properties = {}) {
    ...
    const height = positiveNumberFromProperties(properties, ['height', 'h'], 1, 'height')
  ...
  cone (properties = {}) {
    const openCascade = this.openCascade
    const radius1 = nonNegativeNumberFromProperties(properties, ['radius1', 'r1', 'bottomRadius', 'radius', 'r'], 1, 'radius1')
    const radius2 = nonNegativeNumberFromProperties(properties, ['radius2', 'r2', 'topRadius'], 0, 'radius2')
    const height = positiveNumberFromProperties(properties, ['height', 'h'], 1, 'height')

    if (radius1 === 0 && radius2 === 0) {
      throw new RangeError('cone radius1 or radius2 must be a positive finite number')
    }
  ...
  torus (properties = {}) {
    return primitiveShape(
      ...
      [
        positiveNumberFromProperties(properties, ['majorRadius', 'radius1', 'r1', 'major'], 1, 'major radius'),
        positiveNumberFromProperties(properties, ['minorRadius', 'radius2', 'r2', 'minor', 'tubeRadius'], 0.25, 'minor radius')
      ]
    )
  }
  ...
  translate (properties = {}, children = []) {
    ...
    if (hasRenderableMesh(children)) {
      return shape('transform', 'sol-translate', properties, children, null, [])
    }
  ...
  rotate (properties = {}, children = []) {
    ...
    if (hasRenderableMesh(children)) {
      return shape('transform', 'sol-rotate', properties, children, null, [])
    }
  ...
  scale (properties = {}, children = []) {
    ...
    if (hasRenderableMesh(children)) {
      return shape('transform', 'sol-scale', properties, children, null, [])
    }
  ...
  union (properties = {}, children = []) {
    if (hasRenderableMesh(children)) {
      return shape('operation', 'sol-union', properties, children, null, [])
    }

    const result = combineBoolean(this.openCascade, ['BRepAlgoAPI_Fuse_3', 'BRepAlgoAPI_Fuse'], children, properties)

    return shape('operation', 'sol-union', properties, children, result.value, result.handles)
  }

  difference (properties = {}, children = []) {
    const result = combineBoolean(this.openCascade, ['BRepAlgoAPI_Cut_3', 'BRepAlgoAPI_Cut'], children, properties)

    return shape('operation', 'sol-difference', properties, children, result.value, result.handles)
  }

  intersection (properties = {}, children = []) {
    const result = combineBoolean(this.openCascade, ['BRepAlgoAPI_Common_3', 'BRepAlgoAPI_Common'], children, properties)
  ...
  group (properties = {}, children = []) {
    if (hasRenderableMesh(children)) {
      return shape('operation', 'sol-group', properties, children, null, [])
    }

    const result = makeCompoundShape(this.openCascade, children)
  ...
  fillet (properties = {}, children = []) {
    ...
    const result = combined.value
      ? makeEdgeFeatureShape(openCascade, 'fillet', properties, combined.value, combined.handles, combined.topology)
      : combined
  ...
  chamfer (properties = {}, children = []) {
    ...
    const combined = combineChildren(openCascade, children)
    const result = combined.value
      ? makeEdgeFeatureShape(openCascade, 'chamfer', properties, combined.value, combined.handles, combined.topology)
      : combined
  ...
  stl (properties = {}, children = []) {
    ...
    const entry = shape('external', 'sol-stl', properties, children, result.value, result.handles)
  ...
  toStep (entry, options = {}) {
    return exportStepShape(this.openCascade, exportShapeValue(entry), options)
  }

  toStl (entry, options = {}) {
    return exportStlShape(this.openCascade, exportShapeValue(entry), options)
  }

  toBrep (entry, options = {}) {
    return exportBrepShape(this.openCascade, exportShapeValue(entry), options)
  }
...
function makeCompoundShape (openCascade, children) {
  const entries = children.filter(shapeValue)
  const values = entries.map(shapeValue)
  ...
  if (values.length === 1) {
    return { value: values[0], handles: [], topology: entries[0].topology || null }
  }

  const compound = constructOpenCascadeBinding(openCascade, ['TopoDS_Compound'], [])
  const builder = constructOpenCascadeBinding(openCascade, ['TopoDS_Builder'], [])

  if (typeof builder.MakeCompound !== 'function' || typeof builder.Add !== 'function') {
    throw new TypeError('OpenCascade TopoDS_Builder does not support compound construction')
  }

  builder.MakeCompound(compound)

  for (const value of values) {
    builder.Add(compound, value)
  }

  return { value: compound, handles: [compound, builder] }
}

function combineBoolean (openCascade, names, children, properties = {}) {
  const entries = children.filter(shapeValue)
  const values = entries.map(shapeValue)
  ...
  return values.slice(1).reduce((result, next) => {
    const step = makeBooleanShape(openCascade, names, result.value, next, properties)
...
function makeBooleanShape (openCascade, names, left, right, properties = {}) {
  let progress = constructOptionalOpenCascadeBinding(openCascade, ['Message_ProgressRange_1', 'Message_ProgressRange'], [])
  let builder

  if (progress) {
    try {
      builder = constructOpenCascadeBinding(openCascade, names, [left, right, progress])
    } catch (error) {
  ...
  if (!builder) {
    builder = constructOpenCascadeBinding(openCascade, names, [left, right])
  }

  configureBooleanBuilder(openCascade, builder, properties)
  buildBooleanBuilder(builder, progress)

  return {
    value: extractShape(builder),
    handles: progress ? [left, progress, builder] : [left, builder]
  }
}

function configureBooleanBuilder (openCascade, builder, properties) {
  const fuzzy = properties.fuzzy ?? properties.tolerance

  if (fuzzy !== undefined) {
    callBooleanSetter(builder, ['SetFuzzyValue'], [Number(fuzzy)])
  }

  if (properties.glue !== undefined) {
    callBooleanSetter(builder, ['SetGlue'], [booleanGlueMode(openCascade, properties.glue)])
  }

  if (properties.check !== undefined) {
    callBooleanSetter(builder, ['SetCheckInverted'], [Boolean(properties.check)])
  }

  if (properties.nonDestructive !== undefined) {
    callBooleanSetter(builder, ['SetNonDestructive'], [Boolean(properties.nonDestructive)])
  }

  if (properties.parallel !== undefined) {
    callBooleanSetter(builder, ['SetRunParallel'], [Boolean(properties.parallel)])
  }
}

function callBooleanSetter (builder, names, args) {
  for (const name of names) {
...
function booleanGlueMode (openCascade, value) {
  const mode = String(value === true ? 'full' : value).toLowerCase()
  const suffix = {
    false: 'Off',
    off: 'Off',
    shift: 'Shift',
  ...
  return enumValue(openCascade, 'BOPAlgo_GlueEnum', `BOPAlgo_Glue${suffix}`, suffix)
}

function buildBooleanBuilder (builder, progress) {
...
function makeShellShape (openCascade, properties, children) {
  ...
  const faceResult = collectSelectedFaces(openCascade, combined.value, properties, combined.topology)
...
function makeEdgeFeatureShape (openCascade, type, properties, value, handles = [], topology = null) {
  const featureHandles = []
  ...
  try {
    ...
    const edges = selectShapeEdges(edgeResult.edges, properties, topology?.edges)
...
function selectShapeEdges (edges, properties, namedSelectors = {}) {
  const indexes = selectedEdgeIndexes(properties, edges.length, namedSelectors)
...
function selectedEdgeIndexes (properties, count, namedSelectors = {}) {
  ...
  return resolveTopologySelector(selection, count, namedSelectors)
}
...
function collectSelectedFaces (openCascade, value, properties, topology = null) {
  const faceResult = collectShapeFaces(openCascade, value)
  const indexes = selectedFaceIndexes(properties, faceResult.faces.length, topology?.faces)
...
function selectedFaceIndexes (properties, count, namedSelectors = {}) {
...
function importStlShape (openCascade, properties) {
  ...
  try {
    const read = reader.Read(value, file.path)

    const mesh = file.source === undefined ? null : parseStlMesh(file.source)

    if (read !== false && !isNullShape(value)) {
      return { value, mesh, handles: [reader] }
    }

    if (mesh) {
      return { value: null, mesh, handles: [reader] }
    }

    throw new Error(`Unable to import STL ${file.path}`)
  } finally {
    removeImportFile(openCascade, file)
  }
}
...
function importFileFromProperties (openCascade, properties, extension) {
  ...
  if (source !== undefined) {
    const temporaryPath = properties.path ?? properties.file ?? `/solidark-import.${extension}`

    writeOpenCascadeFile(openCascade, temporaryPath, source)
    return { path: temporaryPath, source, temporary: true }
  }
...
function exportStepShape (openCascade, value, options) {
  if (!value) {
    return ''
  }

  ensureExportFileSystem(openCascade)
  const path = exportPathFromOptions(options, 'step')
  const before = exportDirectoryEntries(openCascade)
  const writer = constructOpenCascadeBinding(openCascade, ['STEPControl_Writer_1', 'STEPControl_Writer'], [])

  try {
    transferStepShape(openCascade, writer, value, options)
    writeExportFile(writer, path, 'STEPControl_Writer')
    return readExportFile(openCascade, path, options, before)
  } finally {
    removeExportFiles(openCascade, path, before)
  }
}

function transferStepShape (openCascade, writer, value, options) {
  const mode = stepTransferMode(openCascade, options)

  if (typeof writer.Transfer_1 === 'function') {
    callStepTransfer(writer.Transfer_1.bind(writer), value, mode)
    return
  }

  if (typeof writer.Transfer === 'function') {
    callStepTransfer(writer.Transfer.bind(writer), value, mode)
    return
  }

  throw new TypeError('OpenCascade STEPControl_Writer does not support Transfer')
}

function callStepTransfer (transfer, value, mode) {
  try {
    transfer(value, mode, true)
  } catch (error) {
    if (!isBindingArgumentCountError(error)) {
      throw error
    }
...
function stepTransferMode (openCascade, options) {
  const mode = String(options.mode || options.type || 'as-is').toLowerCase()
  const member = {
    asis: 'STEPControl_AsIs',
    'as-is': 'STEPControl_AsIs',
    faceted: 'STEPControl_FacetedBrep',
    'faceted-brep': 'STEPControl_FacetedBrep',
    manifold: 'STEPControl_ManifoldSolidBrep',
    shell: 'STEPControl_ShellBasedSurfaceModel'
  }[mode] || 'STEPControl_AsIs'

  return enumValue(openCascade, 'STEPControl_StepModelType', member, 0)
}

function exportStlShape (openCascade, value, options) {
  if (!value) {
    return ''
  }

  ensureExportFileSystem(openCascade)
  const path = exportPathFromOptions(options, 'stl')
  const writer = constructOpenCascadeBinding(openCascade, ['StlAPI_Writer_1', 'StlAPI_Writer'], [])

  try {
    if (writeStlShape(writer, value, path) && exportFileExists(openCascade, path)) {
      return readExportFile(openCascade, path, options)
    }
  } finally {
    removeExportFile(openCascade, path)
  }

  return asciiStlFromMesh(meshValue(openCascade, 'solidark-export', value, options))
}

function writeStlShape (writer, value, path) {
  if (typeof writer.Write_1 === 'function') {
    return writer.Write_1(value, path) !== false
  }

  if (typeof writer.Write === 'function') {
    return writer.Write(value, path) !== false
  }

  throw new TypeError('OpenCascade StlAPI_Writer does not support Write')
}

function exportBrepShape (openCascade, value, options) {
  if (!value) {
    return ''
  }

  ensureExportFileSystem(openCascade)
  const path = exportPathFromOptions(options, 'brep')
  const before = exportDirectoryEntries(openCascade)

  try {
    writeBrepShape(openCascade, value, path)
    return readExportFile(openCascade, path, options, before)
  } finally {
    removeExportFiles(openCascade, path, before)
  }
...
function writeBrepShape (openCascade, value, path) {
  const tools = openCascade.BRepTools

  if (tools && typeof tools.Write_2 === 'function') {
    const progress = constructOptionalOpenCascadeBinding(openCascade, ['Handle_Message_ProgressIndicator_1'], [])

    try {
      if (progress) {
        tools.Write_2(value, path, progress)
      } else {
        tools.Write_2(value, path)
      }
    } catch (error) {
      if (!progress || !isBindingArgumentCountError(error)) {
        throw error
      }
  ...
  if (tools && typeof tools.Write_1 === 'function') {
    tools.Write_1(value, path)
    return
  }

  if (tools && typeof tools.Write === 'function') {
    tools.Write(value, path)
    return
  }

  throw new TypeError('OpenCascade BRepTools.Write is required for BREP export')
}

function exportPathFromOptions (options, extension) {
  return String(options.path || options.file || `/solidark-export.${extension}`)
}

function writeExportFile (writer, path, name) {
  if (typeof writer.Write === 'function') {
    writer.Write(path)
    return
  }
  ...
  throw new TypeError(`OpenCascade ${name} does not support Write`)
}

function readExportFile (openCascade, path, options, before = null) {
  try {
    return openCascade.FS.readFile(path, readFileOptions(options))
  } catch (error) {
    const fallback = readNewExportFile(openCascade, before, options)
...
function exportFileExists (openCascade, path) {
  try {
    if (typeof openCascade.FS.stat === 'function') {
      openCascade.FS.stat(path)
    } else {
      openCascade.FS.readFile(path)
    }
...
function exportDirectoryEntries (openCascade) {
  if (typeof openCascade.FS.readdir !== 'function') {
    return null
  }

  return new Set(openCascade.FS.readdir('/'))
}

function readNewExportFile (openCascade, before, options) {
  if (!before || typeof openCascade.FS.readdir !== 'function') {
    return null
  }

  for (const entry of openCascade.FS.readdir('/')) {
    if (before.has(entry)) {
      continue
    }

    try {
      return openCascade.FS.readFile(`/${entry}`, readFileOptions(options))
    } catch {}
  }
...
function ensureExportFileSystem (openCascade) {
  if (!openCascade.FS || typeof openCascade.FS.readFile !== 'function') {
    throw new TypeError('OpenCascade virtual file system is required for exports')
  }
}

function readFileOptions (options) {
  return options.binary ? undefined : { encoding: 'utf8' }
}

function removeExportFiles (openCascade, path, before) {
  removeExportFile(openCascade, path)
  ...
  for (const entry of openCascade.FS.readdir('/')) {
    if (!before.has(entry)) {
      removeExportFile(openCascade, `/${entry}`)
    }
  }
}

function removeExportFile (openCascade, path) {
  if (openCascade.FS && typeof openCascade.FS.unlink === 'function') {
    try {
      openCascade.FS.unlink(path)
    } catch {
      // The export writer may fail before creating a virtual file.
    }
  }
}

function asciiStlFromMesh (mesh) {
  const lines = ['solid solidark']
...
function exportShapeValue (entry) {
...
function shape (category, tag, properties, children, value, handles = []) {
  ...
  const topology = topologyForShape(properties, children, category)
...
function hasRenderableMesh (entries = []) {
  return entries.some((entry) => {
    return Boolean(entry?.mesh) || hasRenderableMesh(entry?.children || [])
  })
}

function isNullShape (value) {
  return typeof value?.IsNull === 'function' && value.IsNull()
}

function topologyForShape (properties, children, category) {
  return parseTopologyNames(properties.topology || properties.topologyNames) ||
    inheritedTopology(children, category)
}

function inheritedTopology (children, category) {
  if (category !== 'transform' || children.length !== 1) {
    return null
  }

  return children[0]?.topology || null
}
...
function meshEntry (openCascade, entry, options, context = {}) {
  ...
  const transform = meshTransformForEntry(entry, context.transform)
  const children = entry.children || []

  if (shouldMeshChildren(entry, children)) {
    return children.flatMap((child) => meshEntry(openCascade, child, options, { styling, transform }))
  }

  if (entry.mesh) {
    return [applyMeshStyling(transformMesh(entry.mesh, transform), styling)]
  }
...
function meshTransformForEntry (entry, inherited = { offset: [0, 0, 0], rotation: [0, 0, 0], scale: [1, 1, 1] }) {
  if (entry.tag === 'sol-translate') {
    return {
      ...inherited,
      offset: addVectors(inherited.offset, vectorFromProperties(entry.properties || {}))
    }
  }

  if (entry.tag === 'sol-rotate') {
    return {
      ...inherited,
      rotation: addVectors(inherited.rotation, vectorFromProperties(entry.properties || {}))
    }
  }

  if (entry.tag === 'sol-scale') {
    return {
      ...inherited,
      scale: multiplyVectors(inherited.scale, scaleVectorFromProperties(entry.properties || {}))
    }
  }
...
function transformMesh (mesh, transform = { offset: [0, 0, 0], rotation: [0, 0, 0], scale: [1, 1, 1] }) {
  if (!transform.offset.some(Boolean) &&
    !transform.rotation.some(Boolean) &&
  ...
  return {
    ...
    vertices: mesh.vertices.map((vertex) => {
      return addVectors(rotateVector(multiplyVectors(vertex, transform.scale), transform.rotation), transform.offset)
    }),
...
function rotateVector (vector, rotation) {
  return [
    ...
    rotateAroundY,
    rotateAroundZ
  ].reduce((current, rotate, index) => {
    return rotation[index] ? rotate(current, degreesToRadians(rotation[index])) : current
  }, vector)
}

function rotateAroundX ([x, y, z], radians) {
...
function rotateAroundY ([x, y, z], radians) {
  const cos = Math.cos(radians)
  const sin = Math.sin(radians)
...
function rotateAroundZ ([x, y, z], radians) {
  const cos = Math.cos(radians)
  const sin = Math.sin(radians)
...
function sizeFromProperties (properties) {
  if (properties.size !== undefined) {
    ...
    return positiveNumberList([width, depth, height], 'size')
  }

  const width = positiveNumberFromProperties(properties, ['width', 'x'], 1, 'width')
  const depth = positiveNumberFromProperties(properties, ['depth', 'y'], width, 'depth')
  const height = positiveNumberFromProperties(properties, ['height', 'z'], depth, 'height')

  return [width, depth, height]
}

function radiusFromProperties (properties) {
  return positiveNumberFromProperties(properties, ['radius', 'r'], 1, 'radius')
}
...
function ellipseRadiiFromProperties (properties) {
  ...
  return [
    positiveNumberFromProperties(properties, ['radiusX', 'rx', 'majorRadius', 'major', 'x'], radius, 'radiusX'),
    positiveNumberFromProperties(properties, ['radiusY', 'ry', 'minorRadius', 'minor', 'y'], radius, 'radiusY')
  ]
}
...
function positiveNumberFromProperties (properties, names, fallback, label) {
  return positiveNumber(numberFromProperties(properties, names, fallback), label)
}

function nonNegativeNumberFromProperties (properties, names, fallback, label) {
  return nonNegativeNumber(numberFromProperties(properties, names, fallback), label)
}

function positiveNumberList (values, label) {
  return values.map((value) => positiveNumber(value, label))
}
