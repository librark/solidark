...
it('builds centered and uncentered primitive OpenCascade shapes', () => {
  ...
  const namedTopology = kernel.cuboid({ topology: 'rounded:edge:0..3 lid:face:last' })
  assert.deepEqual(namedTopology.topology, {
    edges: { rounded: '0..3' },
    faces: { lid: 'last' }
  })
  const movedTopology = kernel.translate({ by: '1 0 0' }, [namedTopology])
  assert.deepEqual(movedTopology.topology, namedTopology.topology)
})
...
it('validates primitive dimensions before constructing OpenCascade shapes', () => {
  const { openCascade } = createFakeOpenCascade()
  const kernel = createOpenCascadeAdapter(openCascade)

  assert.throws(() => kernel.cuboid({ size: '1 0 1' }), /size must be a positive finite number/)
  assert.throws(() => kernel.sphere({ radius: Number.NaN }), /radius must be a positive finite number/)
  assert.throws(() => kernel.cylinder({ height: 0 }), /height must be a positive finite number/)
  assert.throws(() => kernel.cone({ radius1: 0, radius2: 0 }), /cone radius1 or radius2/)
  assert.throws(() => kernel.cone({ radius2: -1 }), /radius2 must be a non-negative finite number/)
  assert.throws(() => kernel.torus({ minorRadius: Number.POSITIVE_INFINITY }), /minor radius must be a positive finite number/)
  assert.throws(() => kernel.circle({ radius: -1 }), /radius must be a positive finite number/)
  assert.throws(() => kernel.ellipse({ radiusX: 0 }), /radiusX must be a positive finite number/)
  assert.throws(() => kernel.rectangle({ width: 'wide' }), /width must be a positive finite number/)
})
...
it('builds OpenCascade sketch primitive wires', () => {
  ...
  const ellipse = kernel.ellipse({ radiusX: 6, radiusY: 3 })
  assert.equal(ellipse.tag, 'sol-ellipse')
  assert.equal(ellipse.category, 'sketch')
  assert.equal(ellipse.value.edges.length, 1)
  assert.equal(ellipse.value.edges[0].builder.args[0].kind, 'gp_Elips')
  assert.equal(ellipse.value.edges[0].builder.args[0].majorRadius, 6)
  assert.equal(ellipse.value.edges[0].builder.args[0].minorRadius, 3)
  assert.deepEqual(ellipse.value.edges[0].builder.args[0].axis.xDirection.args, [1, 0, 0])

  const yMajorEllipse = kernel.ellipse({ rx: 2, ry: 5 })
  assert.equal(yMajorEllipse.value.edges[0].builder.args[0].majorRadius, 5)
  assert.equal(yMajorEllipse.value.edges[0].builder.args[0].minorRadius, 2)
  assert.deepEqual(yMajorEllipse.value.edges[0].builder.args[0].axis.xDirection.args, [0, 1, 0])
...
it('supports OpenCascade boolean operations without progress range bindings', () => {
  const { openCascade } = createFakeOpenCascade({ booleanBuildArity: 0, progressRange: false })
  const kernel = createOpenCascadeAdapter(openCascade)
  const first = kernel.sphere({ radius: 1 })
  ...
  assert.equal(union.value.builder.built, true)
  assert.deepEqual(union.value.builder.buildArgs, [])
  assert.equal(union.handles.length, 2)
})

it('passes OpenCascade boolean operation settings to supported builders', () => {
  const { openCascade } = createFakeOpenCascade({ booleanBuildArity: 1 })
  const kernel = createOpenCascadeAdapter(openCascade)
  const first = kernel.sphere({ radius: 1 })
  const second = kernel.cuboid({ size: [2], centered: false })
  ...
  assert.equal(union.value.builder.built, true)
  assert.equal(union.value.builder.buildArgs[0].kind, 'progress')
  assert.deepEqual(union.value.builder.settings, {
    checkInverted: true,
    fuzzyValue: 0.2,
    glue: 'glue-shift',
    nonDestructive: true,
  ...
  const difference = kernel.difference({
    check: false,
    glue: false,
    tolerance: 0.05
  }, [first, second])

  assert.equal(difference.value.builder.settings.fuzzyValue, 0.05)
  assert.equal(difference.value.builder.settings.glue, 'glue-off')
  assert.equal(difference.value.builder.settings.checkInverted, false)

  delete openCascade.BOPAlgo_GlueEnum.BOPAlgo_GlueFull
  const intersection = kernel.intersection({ glue: true }, [first, second])
  assert.equal(intersection.value.builder.settings.glue, 'Full')
})

it('falls back when OpenCascade boolean builders do not support option helpers', () => {
  const { openCascade } = createFakeOpenCascade({
    booleanBuildArity: 0,
    booleanSetters: false
  })
  const kernel = createOpenCascadeAdapter(openCascade)
  const first = kernel.sphere({ radius: 1 })
  const second = kernel.cuboid({ size: [2], centered: false })
  const union = kernel.union({ glue: 'unexpected' }, [first, second])
...
it('retries OpenCascade boolean builds when progress ranges are rejected', () => {
  const { openCascade, handles } = createFakeOpenCascade({ booleanBuildArity: 0 })
  const kernel = createOpenCascadeAdapter(openCascade)
  const first = kernel.sphere({ radius: 1 })
  const second = kernel.cuboid({ size: [2], centered: false })
  const union = kernel.union({}, [first, second])
...
it('rethrows non-arity errors from OpenCascade boolean builds', () => {
  const { openCascade } = createFakeOpenCascade({ booleanBuildArity: 'fatal-error' })
  const kernel = createOpenCascadeAdapter(openCascade)
  const first = kernel.sphere({ radius: 1 })
  const second = kernel.cuboid({ size: [2], centered: false })
...
it('supports OpenCascade intersections and empty feature wrappers', () => {
  ...
  assert.equal(group.value.kind, 'compound')
  assert.deepEqual(group.value.children, [first.value, second.value])

  const emptyGroup = kernel.group({}, [])
  assert.equal(emptyGroup.value, null)

  const singleGroup = kernel.group({}, [first])
  assert.equal(singleGroup.value, first.value)
...
it('reports incomplete OpenCascade compound builders', () => {
  {
    const { openCascade } = createFakeOpenCascade()
    const kernel = createOpenCascadeAdapter(openCascade)
    const first = kernel.sphere()
    const second = kernel.cuboid()

    openCascade.TopoDS_Builder = class IncompleteTopologyBuilder {
      Add () {}
    }

    assert.throws(
      () => kernel.group({}, [first, second]),
...
it('applies OpenCascade fillets and chamfers to selected edges', () => {
  ...
  const { value: rangedValue } = solidShape(5)
  const rangedChamfer = kernel.chamfer({ edges: '1..3 -1 first', d: 1 }, [{ value: rangedValue }])
  assert.deepEqual(
    rangedChamfer.value.builder.edges.map((entry) => entry.edge.source),
    rangedValue.edges
  )

  const descendingFillet = kernel.fillet({ edges: '3-1', radius: 1 }, [{ value: rangedValue }])
  assert.deepEqual(
    descendingFillet.value.builder.edges.map((entry) => entry.edge.source),
    rangedValue.edges.slice(1, 4)
  )

  const nestedSelection = kernel.fillet({ edges: [['last'], 0], radius: 1 }, [{ value: rangedValue }])
  assert.deepEqual(
    nestedSelection.value.builder.edges.map((entry) => entry.edge.source),
    [rangedValue.edges[0], rangedValue.edges[4]]
  )

  const invalidSelection = kernel.chamfer({ edges: 'unknown 12' }, [{ value: rangedValue }])
  assert.equal(invalidSelection.value, rangedValue)

  const namedFillet = kernel.fillet({ edges: 'rounded last' }, [{
    value: rangedValue,
    topology: { edges: { rounded: '1..2' } }
  }])
  assert.deepEqual(
    namedFillet.value.builder.edges.map((entry) => entry.edge.source),
    [rangedValue.edges[1], rangedValue.edges[2], rangedValue.edges[4]]
  )
})
...
it('builds OpenCascade sketch arcs from three declarative points', () => {
  const { openCascade } = createFakeOpenCascade()
  const kernel = createOpenCascadeAdapter(openCascade)
  const sketch = kernel.sketch({}, [
    kernel.move({ point: [1, 0] }),
    kernel.arc({ through: [0, 1], point: [-1, 0] }),
    kernel.close()
  ])
  const clockwise = kernel.sketch({}, [
    kernel.move({ point: [1, 0] }),
    kernel.arc({ through: [0, -1], end: [-1, 0] })
  ])
  const missingThrough = [
    kernel.move({ point: [0, 0] }),
    kernel.arc({ point: [1, 0] })
  ]
  const collinear = [
    kernel.move({ point: [0, 0] }),
    kernel.arc({ through: [1, 0], point: [2, 0] })
  ]
  const ignoredInitialArc = kernel.sketch({}, [
    kernel.arc({ through: [0, 1], point: [1, 1] }),
    kernel.move({ point: [0, 0] }),
    kernel.line({ point: [1, 0] })
  ])

  assert.equal(sketch.value.edges.length, 2)
  assert.equal(sketch.value.edges[0].builder.args[0].kind, 'gp_Circ')
  assert.equal(sketch.value.edges[0].builder.args[0].radius, 1)
  assert.equal(sketch.value.edges[0].builder.args[1], 0)
  assert.equal(sketch.value.edges[0].builder.args[2], Math.PI)
  assert.equal(clockwise.value.edges[0].builder.args[2], -Math.PI)
  assert.equal(ignoredInitialArc.value.edges.length, 1)
  assert.throws(() => kernel.sketch({}, missingThrough), /through point/)
  assert.throws(() => kernel.sketch({}, collinear), /non-collinear/)
})
...
it('requires at least two OpenCascade loft sections', () => {
  const { openCascade } = createFakeOpenCascade()
  const kernel = createOpenCascadeAdapter(openCascade)
  const sketch = kernel.sketch({}, [
    kernel.move({ point: [0, 0] }),
  ...
  assert.throws(
    () => kernel.loft({}, []),
    /sol-loft requires at least two profile children/
  )
  assert.throws(
    () => kernel.loft({}, [sketch]),
    /sol-loft requires at least two profile children/
...
it('resolves inherited and direct styling metadata on OpenCascade meshes', () => {
  const { openCascade, meshShape } = createFakeOpenCascade()
  const kernel = createOpenCascadeAdapter(openCascade)
  const { value: inheritedValue } = meshShape()
  const { value: directValue } = meshShape()
  const { value: materialValue } = meshShape()
  const { value: groupValue } = meshShape()
  const { value: groupedFirstValue } = meshShape()
  const { value: groupedSecondValue } = meshShape()
  const meshes = kernel.toMesh({
    tag: 'sol-color',
    styling: { material: 'painted' },
    properties: { color: '#336699' },
    children: [
      { tag: 'sol-inherited', value: inheritedValue, properties: {}, children: [] },
      { tag: 'sol-direct', value: directValue, properties: { colour: 'tomato' }, children: [] }
    ]
  })

  assert.equal(meshes[0].tag, 'sol-inherited')
  assert.equal(meshes[0].color, '#336699')
  assert.deepEqual(meshes[0].styling, { material: 'painted', color: '#336699' })
  assert.equal(meshes[1].tag, 'sol-direct')
  assert.equal(meshes[1].color, 'tomato')
  assert.deepEqual(meshes[1].styling, { material: 'painted', color: 'tomato' })

  const [materialOnly] = kernel.toMesh({
    tag: 'sol-material',
    value: materialValue,
    styling: { material: 'raw' },
    properties: {},
    children: []
  })

  assert.deepEqual(materialOnly.styling, { material: 'raw' })
  assert.equal('color' in materialOnly, false)

  const grouped = kernel.toMesh({
    tag: 'sol-group',
    value: groupValue,
    styling: { color: 'gray' },
    properties: {},
    children: [
      { tag: 'sol-first', value: groupedFirstValue, properties: { color: 'red' }, children: [] },
      { tag: 'sol-second', value: groupedSecondValue, properties: {}, children: [] }
    ]
  })

  assert.deepEqual(grouped.map((mesh) => mesh.tag), ['sol-first', 'sol-second'])
  assert.deepEqual(grouped.map((mesh) => mesh.color), ['red', 'gray'])

  const implicitUnion = kernel.toMesh({
    tag: 'sol-union',
    value: groupValue,
    properties: { implicit: true },
    children: [
      { tag: 'sol-union-first', value: groupedFirstValue, properties: { color: 'red' }, children: [] },
      { tag: 'sol-union-second', value: groupedSecondValue, properties: {}, children: [] }
    ]
  })

  assert.deepEqual(implicitUnion.map((mesh) => mesh.tag), ['sol-union-first', 'sol-union-second'])
  assert.deepEqual(implicitUnion.map((mesh) => mesh.color), ['red', undefined])
})
...
it('builds OpenCascade profile, extrusion, revolution, and sweep features', () => {
  ...
  assert.throws(
    () => kernel.face({}, []),
    /sol-face requires one profile child/
  )
  assert.throws(
    () => kernel.extrude({}, []),
    /sol-extrude requires one profile child/
  )
  assert.throws(
    () => kernel.revolve({}, []),
    /sol-revolve requires one profile child/
  )
  assert.throws(
    () => kernel.sweep({}, []),
    /sol-sweep requires a profile child and a path child/
  )
  assert.throws(
    () => kernel.sweep({}, [sketch]),
    /sol-sweep requires a profile child and a path child/
  ...
  assert.throws(
    () => kernel.sweep({ path: 4 }, [sketch, path]),
    /sol-sweep path index 4/
  )
})
...
it('builds OpenCascade offset, shell, section, and import features', () => {
  ...
  const namedFacesShell = kernel.shell({ faces: 'first last' }, [{ value }])
  assert.deepEqual(
    namedFacesShell.value.builder.thick[1].items.map((face) => face.source),
    [value.faces[0], value.faces[2]]
  )

  const rangeFacesShell = kernel.shell({ faces: '2-1' }, [{ value }])
  assert.deepEqual(
    rangeFacesShell.value.builder.thick[1].items.map((face) => face.source),
    [value.faces[1], value.faces[2]]
  )

  const invalidFacesShell = kernel.shell({ faces: 'not-a-face 12..20' }, [{ value }])
  assert.equal(invalidFacesShell.value.builder.thick[1].items.length, 0)

  const namedShell = kernel.shell({ faces: 'lid' }, [{
    value,
    topology: { faces: { lid: 'first last' } }
  }])
  assert.deepEqual(
    namedShell.value.builder.thick[1].items.map((face) => face.source),
    [value.faces[0], value.faces[2]]
  )
  ...
  const fetchedStl = kernel.stl({ data: new Uint8Array([1, 2, 3]), src: 'https://example.test/fetched.stl' })
  assert.equal(fetchedStl.value.import.path, '/solidark-import.stl')
  assert.equal(openCascade.FS.files['https://example.test/fetched.stl'], undefined)

  const namedStl = kernel.stl({ data: 'solid named', file: '/named-import.stl' })
  assert.equal(namedStl.value.import.path, '/named-import.stl')
  assert.equal(openCascade.FS.files['/named-import.stl'], undefined)

  const virtualStl = kernel.stl({ path: '/virtual-import.stl' })
  assert.equal(virtualStl.value.import.path, '/virtual-import.stl')
  assert.equal(virtualStl.mesh, undefined)

  {
    openCascade.TopoDS_Shape = class NonNullImportedShape {
      IsNull () {
        return false
      }
    }
    const nonNullStl = kernel.stl({
      data: `
        solid non-null
          facet normal 0 0 1
            outer loop
    ...
    assert.equal(nonNullStl.value.import.path, '/non-null.stl')
    assert.deepEqual(nonNullStl.mesh.triangles, [[0, 1, 2]])
    assert.equal(kernel.group({}, [nonNullStl, kernel.cuboid({})]).value, null)
    assert.equal(kernel.union({ implicit: true }, [nonNullStl, kernel.cuboid({})]).value, null)
  }

  {
    openCascade.TopoDS_Shape = class NullImportedShape {
      IsNull () {
        return true
      }
    }
    openCascade.StlAPI_Reader = class MeshFallbackStlReader {
      Read () {
        return false
      }
    }
    const meshStl = kernel.stl({
      data: `
        solid fallback
          facet normal 0 0 1
            outer loop
    ...
    assert.equal(meshStl.value, null)
    assert.deepEqual(meshStl.mesh.triangles, [[0, 1, 2]])
    assert.deepEqual(kernel.toMesh(meshStl), [meshStl.mesh])
    assert.deepEqual(
      kernel.toMesh(kernel.translate({ by: '2 3 4' }, [meshStl]))[0].vertices,
      [[2, 3, 4], [3, 3, 4], [2, 4, 4]]
    )
    assert.deepEqual(
      kernel.toMesh(kernel.scale({ by: '2 3 4' }, [meshStl]))[0].vertices,
      [[0, 0, 0], [2, 0, 0], [0, 3, 0]]
    )
    assert.deepEqual(
      kernel.toMesh(kernel.translate({ by: '2 3 4' }, [kernel.scale({ by: '2 3 4' }, [meshStl])]))[0].vertices,
      [[2, 3, 4], [4, 3, 4], [2, 6, 4]]
    )
    assert.deepEqual(
      kernel.toMesh(kernel.rotate({ by: '0 0 90' }, [meshStl]))[0].vertices.map(roundCoordinates),
      [[0, 0, 0], [0, 1, 0], [-1, 0, 0]]
    )
    assert.deepEqual(
      kernel.toMesh(kernel.rotate({ by: '90 0 0' }, [meshStl]))[0].vertices.map(roundCoordinates),
      [[0, 0, 0], [1, 0, 0], [0, 0, 1]]
    )
    assert.deepEqual(
      kernel.toMesh(kernel.rotate({ by: '0 90 0' }, [meshStl]))[0].vertices.map(roundCoordinates),
      [[0, 0, 0], [0, 0, -1], [0, 1, 0]]
    )
    assert.deepEqual(kernel.toMesh({ tag: 'sol-translate', children: [meshStl] }), [meshStl.mesh])
    assert.deepEqual(kernel.toMesh({ tag: 'sol-rotate', children: [meshStl] }), [meshStl.mesh])
    assert.deepEqual(kernel.toMesh({ tag: 'sol-scale', children: [meshStl] }), [meshStl.mesh])
    assert.throws(() => kernel.stl({ data: 'solid empty' }), /Unable to import STL/)
  }
  ...
  const stepExport = kernel.toStep({ value }, { mode: 'manifold' })
  assert.equal(stepExport, 'step export /solidark-export.step')
  assert.equal(openCascade.FS.files['/solidark-export.step'], undefined)

  const stlExport = kernel.toStl({ value }, { path: '/custom.stl' })
  assert.equal(stlExport, 'stl export /custom.stl')
  assert.equal(openCascade.FS.files['/custom.stl'], undefined)

  const brepExport = kernel.toBrep({ value })
  assert.equal(brepExport, 'brep export /solidark-export.brep')
  assert.equal(openCascade.FS.files['/solidark-export.brep'], undefined)

  assert.equal(kernel.toStep(null), '')
  assert.equal(kernel.toStl({ value: null }), '')
  assert.equal(kernel.toBrep({ value: null }), '')
})
...
it('supports OpenCascade advanced feature fallback bindings', () => {
...

  {
    ...
    delete openCascade.FS.unlink
    assert.equal(kernel.toStep({ value }, { type: 'shell' }), 'step export /solidark-export.step')
    assert.equal(kernel.toStep('raw-shape'), 'step export /solidark-export.step')
    assert.equal(kernel.toStep({ value }, { mode: 'missing' }), 'step export /solidark-export.step')
    assert.equal(kernel.toBrep(value, { file: '/raw.brep' }), 'brep export /raw.brep')
  }

  {
    ...
    openCascade.STEPControl_Writer_1 = class FallbackStepWriter {
      Transfer (shape, mode) {
        this.transfer = { shape, mode }
      }

      Write_1 (path) {
        openCascade.FS.writeFile(path, `fallback step ${this.transfer.mode}`)
      }
    }

    delete openCascade.STEPControl_StepModelType.STEPControl_FacetedBrep
    assert.equal(kernel.toStep({ value }, { file: '/fallback.step', mode: 'faceted-brep' }), 'fallback step 0')
  }

  {
    ...
    openCascade.StlAPI_Writer_1 = class FallbackStlWriter {
      Write_1 (shape, path) {
        openCascade.FS.writeFile(path, `fallback stl ${shape.kind}`)
      }
    }

    assert.equal(kernel.toStl(value, { binary: true }), 'fallback stl shape')
  }

  {
    ...
    openCascade.BRepTools = {
      Read_2 () {},
      Write_1 (shape, path) {
        openCascade.FS.writeFile(path, `fallback brep ${shape.kind}`)
      }
    }

    assert.equal(kernel.toBrep({ value }), 'fallback brep shape')
  }

  {
    ...
    openCascade.Handle_Message_ProgressIndicator_1 = class ProgressIndicator {
      constructor () {
        this.kind = 'progress-indicator'
        progressHandles.push(this)
      }
    ...
    openCascade.BRepTools = {
      Read_2 () {},
      Write_2 (shape, path, progress) {
        openCascade.FS.writeFile(path, `progress brep ${shape.kind} ${progress.kind}`)
      }
    }

    assert.equal(kernel.toBrep({ value }), 'progress brep shape progress-indicator')
    assert.equal(progressHandles[0].deleted, true)
  }

  {
    ...
    openCascade.BRepTools = {
      Read_2 () {},
      Write (shape, path) {
        openCascade.FS.writeFile(path, `generic brep ${shape.kind}`)
      }
    }

    assert.equal(kernel.toBrep({ value }, { binary: true }), 'generic brep shape')
  }

  {
    ...
    openCascade.BRepTools = {
      Read_2 () {},
      Write_2 () {
        throw new TypeError('missing progress brep write failure')
      }
    }

    assert.throws(
      () => kernel.toBrep({ value }),
      /missing progress brep write failure/
    )
  }

  {
    ...
    openCascade.STEPControl_Writer_1 = class FallbackArityStepWriter {
      Transfer (shape, mode, write) {
        if (write !== undefined) {
          throw new Error('function STEPControl_Writer.Transfer called with 3 arguments, expected 2 args!')
        }
      ...
      Write (path) {
        openCascade.FS.writeFile(path, `arity step ${this.transfer.mode}`)
      }
    }

    assert.equal(kernel.toStep({ value }), 'arity step step-as-is')
  }

  {
    ...
    openCascade.FS.readdir = function readdir () {
      return ['.', '..', ...Object.keys(this.files).map((path) => path.replace(/^\//, ''))]
    }
    openCascade.FS.readFile = function readFile (path) {
    ...
    openCascade.STEPControl_Writer_1 = class GarbledPathStepWriter {
      Transfer () {}

      Write () {
        openCascade.FS.writeFile('/garbled-step-name', 'garbled step export')
      }
    }

    assert.equal(kernel.toStep({ value }), 'garbled step export')
    assert.equal('/garbled-step-name' in openCascade.FS.files, false)
  }

  {
    ...
    openCascade.STEPControl_Writer_1 = class BrokenStepWriter {
      Transfer () {
        throw new TypeError('unexpected step transfer failure')
      }
    }

    assert.throws(
      () => kernel.toStep({ value }),
      /unexpected step transfer failure/
    )
  }

  {
    ...
    openCascade.BRepTools = {
      ...
      Write_2 (shape, path, progress) {
        if (progress) {
          throw new Error('function BRepTools.Write_2 called with 3 arguments, expected 2 args!')
        }

        openCascade.FS.writeFile(path, `arity brep ${shape.kind}`)
      }
    }

    assert.equal(kernel.toBrep({ value }), 'arity brep shape')
    assert.equal(progressHandles[0].deleted, true)
  }

  {
    ...
    openCascade.BRepTools = {
      Read_2 () {},
      Write_2 () {
        openCascade.FS.writeFile('/garbled-brep-name', 'garbled brep export')
      }
    }

    assert.equal(kernel.toBrep({ value }), 'garbled brep export')
    assert.equal('/garbled-brep-name' in openCascade.FS.files, false)
  }

  {
    ...
    openCascade.FS.readFile = function readFile () {
      throw new Error('FS error')
    }
    openCascade.STEPControl_Writer_1 = class MissingStepWriter {
      Transfer () {}
      Write () {}
    }
...

  {
    ...
    openCascade.BRepTools = {
      Read_2 () {},
      Write_2 () {
        openCascade.FS.writeFile('/unreadable-export', 'unreadable')
      }
    }
...

  {
    ...
    openCascade.STEPControl_Writer_1 = class MissingReaddirStepWriter {
      Transfer () {}

      Write () {
        delete openCascade.FS.readdir
      }
    }
...

  {
    ...
    openCascade.BRepTools = {
      Read_2 () {},
      Write_2 () {
        throw new TypeError('unexpected brep write failure')
      }
    }

    assert.throws(
      () => kernel.toBrep({ value }),
      /unexpected brep write failure/
    )
    assert.equal(progressHandles[0].deleted, true)
  }

  {
    ...
    openCascade.StlAPI_Writer_1 = class EmptyStlWriter {
      Write () {
        return false
      }
    }

    assert.match(kernel.toStl({ value }), /^solid solidark\n {2}facet normal /)
  }

  {
    ...
    openCascade.FS.stat = function stat (path) {
      stats.push(path)
    }
    openCascade.StlAPI_Writer_1 = class StatStlWriter {
      Write (shape, path) {
        openCascade.FS.writeFile(path, `stat stl ${shape.kind}`)
      }
    }

    assert.equal(kernel.toStl({ value }), 'stat stl shape')
    assert.deepEqual(stats, ['/solidark-export.stl'])
  }

  {
    ...
    openCascade.FS.stat = function stat () {
      throw new Error('missing export file')
    }
    openCascade.FS.unlink = function unlink () {
      throw new Error('already missing')
    }
    openCascade.StlAPI_Writer_1 = class MissingStlWriter {
      Write () {
        return true
      }
    }
...
it('surfaces unsupported OpenCascade advanced feature inputs', () => {
  ...
  {
    const { openCascade, solidShape } = createFakeOpenCascade()
    const readableKernel = createOpenCascadeAdapter(openCascade)
    const { value } = solidShape()

    delete openCascade.FS.readFile
    assert.throws(
      () => readableKernel.toStl({ value }),
      /virtual file system/
    )
  }
})

it('surfaces unsupported OpenCascade export writer bindings', () => {
  {
    const { openCascade, solidShape } = createFakeOpenCascade()
    const kernel = createOpenCascadeAdapter(openCascade)
    const { value } = solidShape()

    openCascade.STEPControl_Writer_1 = class WriteOnlyStepWriter {
      Write () {}
    }
...

  {
    ...
    openCascade.STEPControl_Writer_1 = class TransferOnlyStepWriter {
      Transfer_1 () {}
    }

    assert.throws(
      () => kernel.toStep({ value }),
      /STEPControl_Writer does not support Write/
    )
  }

  {
    ...
    openCascade.StlAPI_Writer_1 = class BrokenStlWriter {}

    assert.throws(
      () => kernel.toStl({ value }),
      /StlAPI_Writer does not support Write/
    )
  }
...
function roundCoordinates (coordinates) {
  return coordinates.map((coordinate) => Math.round(coordinate * 1e6) / 1e6)
}

function createFakeOpenCascade ({
  booleanArity = null,
  booleanBuildArity = null,
  booleanSetters = true,
  ...
  class Axis2 extends Handle {
    constructor (origin, direction, xDirection = null) {
      super('gp_Ax2', xDirection ? [origin, direction, xDirection] : [origin, direction])
      this.origin = origin
      this.direction = direction
  ...
  class Ellipse extends Handle {
    constructor (axis, majorRadius, minorRadius) {
      super('gp_Elips', [axis, majorRadius, minorRadius])
      this.axis = axis
  ...
  class TopologyBuilder extends Handle {
    constructor () {
      super('topology-builder')
    }
  ...
  class StepWriter extends Handle {
    constructor (...args) {
      super('step-writer', args)
    }

    Transfer_1 (shape, mode) {
      this.transfer = { shape, mode }
    }

    Write (path) {
      openCascade.FS.writeFile(path, `step export ${path}`)
      return 'IFSelect_RetDone'
    }
  }

  class StlWriter extends Handle {
    constructor (...args) {
      super('stl-writer', args)
    }

    Write (shape, path) {
      openCascade.FS.writeFile(path, `stl export ${path}`)
      return true
    }
  }
  ...
  const openCascade = {
    ...
    BRepAlgoAPI_Fuse: createBooleanBuilderClass(Handle, 'fuse', shapeMethod, booleanArity, booleanBuildArity, booleanSetters),
    BRepAlgoAPI_Cut: createBooleanBuilderClass(Handle, 'cut', shapeMethod, booleanArity, booleanBuildArity, booleanSetters),
    BRepAlgoAPI_Common: createBooleanBuilderClass(Handle, 'common', shapeMethod, booleanArity, booleanBuildArity, booleanSetters),
    BRepAlgoAPI_Section_5: SectionBuilder,
    BRepBuilderAPI_MakeEdge: EdgeBuilder,
    BRepBuilderAPI_MakeEdge_8: EdgeBuilder,
    BRepBuilderAPI_MakeEdge_9: EdgeBuilder,
    BRepBuilderAPI_MakeEdge_12: EdgeBuilder,
    ...
    TopoDS_Builder: TopologyBuilder,
    ...
    STEPControl_Writer_1: StepWriter,
    StlAPI_Writer_1: StlWriter,
    gp_Pln_3: Plane,
    gp_Trsf: Transform,
    gp_Vec: Vector,
    gp_Pnt: Point,
  ...
  openCascade.STEPControl_StepModelType = {
    STEPControl_AsIs: 'step-as-is',
    STEPControl_FacetedBrep: 'step-faceted-brep',
    STEPControl_ManifoldSolidBrep: 'step-manifold-solid-brep',
    STEPControl_ShellBasedSurfaceModel: 'step-shell-based-surface-model'
  }

  openCascade.BOPAlgo_GlueEnum = {
    BOPAlgo_GlueOff: 'glue-off',
    BOPAlgo_GlueShift: 'glue-shift',
    BOPAlgo_GlueFull: 'glue-full'
  }
  ...
  openCascade.BRepTools = {
    ...
    Write_2 (shape, path) {
      openCascade.FS.writeFile(path, `brep export ${path}`)
      return true
    }
  }
...
function createBooleanBuilderClass (Handle, kind, shapeMethod, arity, buildArity, setters) {
  ...
  class BooleanBuilder extends Builder {
    ...
    Build (...args) {
      if (buildArity === 'fatal-error') {
        throw new Error('boolean build exploded')
      }

      if (typeof buildArity === 'number' && args.length !== buildArity) {
        throw new Error(`BindingError: invalid number of parameters (${args.length}) - expected (${buildArity}) parameters instead!`)
      }
    ...
    SetFuzzyValue (value) {
      this.settings.fuzzyValue = value
    }

    SetGlue (value) {
      this.settings.glue = value
    }

    SetCheckInverted (value) {
      this.settings.checkInverted = value
    }

    SetNonDestructive (value) {
      this.settings.nonDestructive = value
    }

    SetRunParallel (value) {
      this.settings.runParallel = value
    }
  }

  if (buildArity === null) {
    BooleanBuilder.prototype.Build = null
  }

  if (!setters) {
    BooleanBuilder.prototype.SetFuzzyValue = null
    BooleanBuilder.prototype.SetGlue = null
    BooleanBuilder.prototype.SetCheckInverted = null
    BooleanBuilder.prototype.SetNonDestructive = null
    BooleanBuilder.prototype.SetRunParallel = null
  }
